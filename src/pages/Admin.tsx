...
import { Store, Package, Link2, LogOut, Menu, X } from "lucide-react";
import StoreManager from "@/components/admin/StoreManager";
import ProductManager from "@/components/admin/ProductManager";
import LinkGenerator from "@/components/admin/LinkGenerator";

type Tab = "stores" | "products" | "links";
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-muted-foreground hidden sm:block">
              {user.email}
              {isAdmin && (
                <span className="ml-2 inline-flex items-center rounded-full bg-accent/20 px-2 py-0.5 text-xs font-medium text-accent-foreground">
                  Admin
                </span>
              )}
            </span>
      ...
      <main className="mx-auto max-w-7xl p-4 md:p-6">
        {!isAdmin ? (
          <div className="rounded-lg border bg-card p-8 text-center">
            ...
            <h2 className="text-xl font-semibold mb-2">Sem permissões de administrador</h2>
            <p className="text-muted-foreground">
              A sua conta não tem permissões de administrador. Contacte o administrador para obter acesso.
            </p>
          </div>
        ) : (
          <>
